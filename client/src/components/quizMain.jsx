...
function QuizMain() {
    ...
    const getData=async ()=>{
        const resp=await fetch('http://13.232.129.172:8000/getQuizzes2',{
            method:'GET',
            headers:{
                'Content-Type':'application/json',
    ...
    closeOnClick:true
       ...
       const resp=await fetch('http://13.232.129.172:8000/saveQuiz',{
        method:'POST',
        headers:{
            'Content-Type':'application/json',
            'auth-token':localStorage.getItem('token')
    ...
    <div>
      ...
      <div className='m-0 p-0 hidden lg:block'>
        ...
        {(createEvent||editEvent) ?(
            <div>
                <div className='text-3xl font-semibold'>Add New Quiz</div>
                <div className='mt-5 bg-white rounded-xl flex flex-col items-center justify-center py-20'>
                    <div className='w-full  mt-10'>
                        <div className='mx-auto w-5/6 md:w-1/2 lg:w-1/3'>
                            <div className='text-center'>Quiz Name</div>
                    ...
                    {createEvent && (<button onClick={()=>{handleClick()}}className=' mt-10 px-6 py-2 text-white text-lg font-semibold bg-[#315EFF] rounded-lg'>Save Quiz</button>)}
                    {editEvent && (<button onClick={()=>{handleEditEvent()}}className=' mt-10 px-6 py-2 text-white text-lg font-semibold bg-[#315EFF] rounded-lg'>Edit Quiz</button>)}
                </div>
            </div>
        ):(<div>
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
            ...
            <button onClick={()=>{setCreateEvent(true)}} className='px-6 py-2 text-white text-lg font-semibold bg-[#315EFF] rounded-lg'>Create Quiz</button>
            </div>)
        }
        </div>
